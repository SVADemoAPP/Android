...
import com.hck.imagemap.utils.TransUtil;
        ...
        SurfaceHolder.Callback
{
    ...
    public boolean onTouchEvent(MotionEvent event)
    {
        ...
        case MotionEvent.ACTION_UP:
            ...
            if (deltaX < 10 && deltaY < 10 && null != itemFrame)
            {
                ...
                if (clickPosition == -1)
                    break;
                if (cakeValues.get(clickPosition).value != 100f)
                { // 如果只有一个的情况下,就不高亮
                            ...
                            && textGravity == Gravity.bottom)
                    {
                        ...
                        rotaValueAnimator.setDuration(Math.abs(new Float(
                                (toRotaAngle - startAngle)).intValue()) * 5);
    ...
    private void initRect(float cakeSize)
    {
        ...
        {
            float left = (width * 1.0f - cakeSize) / 2;
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec)
    {
        ...
        if (textGravity == Gravity.bottom)
        {
            ...
            } else
            {
                ...
                {
                    measuredHeight = new Float((cakeSize + detailHeight))
                            .intValue();
    ...
    private static int dip2px(Context context, float dipValue)
    {
        ...
        return new Float((dipValue * scale + 0.5f)).intValue();
    }
    ...
    private void drawItem(float top, int item)
    {
        ...
        lockRect.set(new Float(cakeRect.left).intValue(), 0, new Float(
                cakeRect.right).intValue(), 40 + new Float(cakeRect.bottom)
                .intValue());
        canvas = holder.lockCanvas(lockRect);
        if (null != canvas)
        {
            ...
            for (int i = 0; i < cakeValues.size(); i++)
            {
                ...
                if (i == 0)
                {
                    ...
                    if (i != item && top != 0)
                    {
                        ...
                        paint.setAlpha(new Float((top * 200 / HIGHLIGHT_OFFSET))
                                .intValue());
                        canvas.drawArc(drawRect, startAngle,
    ...
    private void drawItemText(String txt)
    {
        ...
        itemRect.set(detailRect.left, detailRect.top
                + new Float(HIGHLIGHT_OFFSET).intValue(), detailRect.right,
                new Float(HIGHLIGHT_OFFSET).intValue() + detailRect.bottom);
        ...
        if (null != canvas)
        {
            ...
            StaticLayout layout = new StaticLayout(txt, textPaint,
                    ...
                    0f, true);
            canvas.translate(
                    itemRect.left + dip2px(getContext(), LEFT_SPACING),
                    itemRect.top);
            layout.draw(canvas);
            holder.unlockCanvasAndPost(canvas);
        }
    ...
    private void drawDetail()
    {
        ...
        if (textGravity == Gravity.right)
        {
            left = new Float((cakeWidth + dip2px(getContext(), LEFT_SPACING)))
                    .intValue();
            top = 0;
        } else
        {
            left = new Float(
                    (cakeRect.left + dip2px(getContext(), LEFT_SPACING)))
                    .intValue();
            top = new Float((cakeHeight + dip2px(getContext(), TOP_SPACING)))
                    .intValue();
        }
        ...
        if (null != canvas)
        {
            ...
            for (int i = 0; i < cakeValues.size(); i++)
            {
                ...
                {
                    drawTxt = new Float(counts[i]).intValue() + unitName;
                }
    ...
    private void drawCakeText(Canvas canvas, int item)
    {
        ...
        if (null != canvas)
        {
            ...
            {
                drawTxt = new Float(counts[item]).intValue() + unitName;
            }
            canvas.drawText(drawTxt, position[0], position[1], paint);
            // holder.unlockCanvasAndPost(canvas);
        }
